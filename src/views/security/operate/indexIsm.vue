  ...
  <div class="table-classic-wrapper">
    ...
    >
      <template slot="custom" slot-scope="scope">
        <myHandle>
          <span title="编辑" @click="edit(scope.data.row)">
            编辑
          </span>
          <span  title="删除" @click="remove(scope.data.row)">
            删除
          </span>
        </myHandle>
      </template>
...
<script>
import { list, del} from "@/api/security/ism";
import edit from "./editIsm";
export default {
  components: { edit },
  data() {
    return {
      ...
      loading: false,
      checked: [
        { key: "制度分类名称", val: "ismName" },
        { key: "添加时间", val: "addTime" },
      ],
      tableData: [],
      // 多选数据暂存数组
      multipleSelection: [],
  ...
  methods: {
    ...
    edit(data) {
      if (data && data.ismId) {
        this.editId = data.ismId;
        this.titleName = "编辑制度分类";
      } else {
        this.editId = null;
        this.titleName = "新增制度分类";
      }
      this.editShow = true;
    },
    //删除
