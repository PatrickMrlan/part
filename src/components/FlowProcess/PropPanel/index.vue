...
<template>
  <el-drawer size="550px" class="drawer" :visible.sync="visible" :show-close="false" style="text-align:left;"
    @close="cancel" v-if="properties">
    <!-- 标题 -->
    <header slot="title" class="header" v-if="value && value.type == 'start' && properties.title">{{ properties.title }}
    </header>
    <header slot="title" class="header" v-else>
      ...
      <el-input size="mini" v-model="properties.title" v-show="titleInputVisible"
        v-clickoutside="_ => titleInputVisible = false" style="z-index:9;max-width: 200px;"></el-input>
      <el-select v-if="isConditionNode()" v-model="properties.priority" size="mini" class="priority-select">
        <el-option v-for="item in priorityLength" :key="item" :value="item - 1" :labcopy-orgel="'优先级' + item"></el-option>
      </el-select>
    </header>
    ...
    <section class="condition-pane" v-if="value && isConditionNode()">
      ...
      <template v-for="(item, index) in pconditions">
        <!-- 计数 -->
        <row-wrapper :key="index" :title="item.label"
          v-if="couldShowIt(item, 'el-input-number', 'fc-date-duration', 'fc-time-duration', 'fc-amount', 'fc-calculate')">
          <num-input :key="index" :title="timeTangeLabel(item)" v-model="item.conditionValue"
            style="padding-right: 6px;"></num-input>
        ...
        <row-wrapper :key="index" :title="item.label" v-if="couldShowIt(item, 'el-radio-group')">
          <el-radio-group v-model="item.conditionValue.value" class="radio-group">
        ...
        <row-wrapper :key="index" :title="item.label" v-if="couldShowIt(item, 'el-select')">
          <el-select v-model="item.conditionValue.value" placeholder="请选择" size="small">
          ...
          <div v-else-if="value.type === 'approver'">
            ...
            <div style="border-bottom: 1px solid #e5e5e5;padding-bottom: 1rem;">
              ...
              <div v-else-if="approverForm.assigneeType === 'director'">
                <div style="font-size: 14px;padding-left: 1rem;">发起人的
                  <el-select v-model="directorLevel" size="small">
                    <el-option v-for="item in 2" :key="item" :label="item === 1 ? '直接主管' : `第${item}级主管`"
              ...
              <div v-else class="option-box">
                <fc-org-select ref="approver-org" buttonType="button" v-model="orgCollection"
            ...
            <div class="option-box" style="border-bottom: 1px solid #e5e5e5;"
              v-if="orgCollection[approverForm.assigneeType] && orgCollection[approverForm.assigneeType].length > 1 || ['optional'].includes(approverForm.assigneeType)">
              <p>多人审批时采用的审批方式</p>
   ...
   <!-- 抄送人 -->
    <section v-if="value && isCopyNode()" style="padding-left: 1rem;">
      <p>抄送人</p>
      <fc-org-select ref="copy-org" v-model="properties.initiator" :tabList="['dep&user']" buttonType="button" title="抄送人" />
      <br>
      <el-checkbox v-model="properties.userOptional">允许发起人自选抄送人</el-checkbox>
    </section>
  <!-- 选择条件弹出框  -->
    <el-dialog title="选择条件" :visible.sync="dialogVisible" width="500px" :append-to-body="true"
...
export default {
  ...
  methods: {
    ...
    conditionNodeComfirm() {
      ...
      this.showingPCons
        ...
        .forEach((t) => {
          ...
          if(cValue.value=== undefined || cValue.cValue === null){
            return
          }
          ...
          if (numberTypeCmp.includes(t.tag)) {
            ...
            nodeContent += `[${t.label} = ${cValue.value}] ` + '\n'
          }
          const res = { formId: t.formId, conditionValue: cValue, code: t.code }
          conditions.push(res)
        }, [])
    ...
    approverNodeComfirm() {
      ...
      this.approverForm.initiator = this.orgCollection[assigneeType]
     ...
     */
    copyNodeConfirm() {
      this.properties.initiator = this.properties.initiator['dep&user']
 ...
 */
    initApproverNodeData() {
      ...
      const initiator = this.approverForm.initiator
      this.resetOrgColl()
      if (Array.isArray(this.approverForm.initiator)) {
        this.orgCollection[this.approverForm.assigneeType] = initiator
      }
    ...
    initCopyNodeData(){
      const initiator = this.value.properties && this.value.properties.initiator
      this.properties.initiator['dep&user'] = Array.isArray(initiator) ? initiator : []
    },
    initCopyNode() {
